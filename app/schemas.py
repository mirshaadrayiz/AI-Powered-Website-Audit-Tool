...
class FactualMetricsSchema(BaseModel):
    ...
    unique_internal_links_count: int = Field(
        ..., description="Number of distinct internal link destinations — repeats to the same URL count once."
    )
    unique_external_links_count: int = Field(
        ..., description="Number of distinct external link destinations — repeats to the same URL count once."
    )
    image_count: int = Field(..., description="Total number of images in the text.")
    image_missing_alt_count: int = Field(
        ..., description="Number of images with no alt attribute at all — a real accessibility gap."
    )
    image_decorative_alt_count: int = Field(
        ...,
        description=(
            "Number of images with alt=\"\" (empty but present)."
        ),
    )
...
class InsightDetailSchema(BaseModel):
    ...
    """
        ...
        The two are separate fields rather than one blob of prose because the
        assignment requires insights to be grounded in the extracted metrics and to
...
class RecommendationReasoningSchema(BaseModel):
    ...
    reasoning: str = Field(
        ...,
        description=(
            "Why this action follows, citing the specific metric value(s) behind it"
        ),
    )
...
class RecommendationSchema(BaseModel):
    ...
    recommendation: List[RecommendationReasoningSchema] = Field(
        ...,
        min_length=3,
        max_length=5,
